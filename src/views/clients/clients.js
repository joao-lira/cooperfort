...
class Clients extends React.Component {
  ...
  delete = async id => {
    try {
      ...
      this.loadClients()
      Notification('success', 'Sucesso!', 'Cliente removido com sucesso')
    } catch (error) {
      Notification(
        'error',
        'Ooops!',
  ...
  handleSendData = async () => {
    ...
    let body = {
      ...
      client_cpf: String(form.client_cpf)
        .replace('.', '')
        .replace('.', '')
        .replace('-', ''),
      client_cep: String(form.client_cep)
        .replace('.', '')
        .replace('-', ''),
    ...
    if (isValid) {
      try {
        ...
        Notification('success', 'Sucesso!', 'Cliente cadastrado com sucesso.')
        this.loadClients()
        ...
        <Modal
          ...
          {this.state.showObject ? (
            <div className="p-4">
              ...
              <pre>
                {JSON.stringify(this.state.form.contacts, undefined, 1)}
              </pre>
              <strong>State EMAILS</strong>
              <pre>{JSON.stringify(this.state.form.emails, undefined, 1)}</pre>
